 ...
 *
 * QEMU patch: dopoll() gates timeout on MC_REALTIME_CTR for correct
 * real-time behavior under -icount shift=0,sleep=off.
 */
	...
	int is_xpg4)
{
	...
	int error;
	/* QEMU real-time tracking for timeout */
	uint32_t dp_rt_start = 0, dp_rt_target_us = 0;
	...
	/* QEMU: capture real-time start for timeout gating.
	 * Cap selticks to 1 so each sleep is ~1 virtual tick (~85ms real
	 * under icount), letting the real-time check control the duration. */
	if (atv && selticks) {
		dp_rt_start = *qemu_rt_ctr;
		dp_rt_target_us = (uint32_t)(atv->tv_sec * 1000000
		                           + atv->tv_usec);
		selticks = 1;
	}
...
quicktry:
	...
	if (atv && (selticks = untimeout(tid)) == 0) {
		/* QEMU: check real time before giving up on timeout */
		if (dp_rt_target_us > 0) {
			uint32_t rt_now = *qemu_rt_ctr;
			uint32_t rt_elapsed = rt_now - dp_rt_start;
			if (rt_elapsed < dp_rt_target_us) {
				uint32_t rt_remain = dp_rt_target_us - rt_elapsed;
				atv->tv_sec = rt_remain / 1000000;
				atv->tv_usec = rt_remain % 1000000;
				/* 1-tick sleep; real-time check controls duration */
				selticks = 1;
				polldelvec(darray, curdat);
				goto retry;
			}
