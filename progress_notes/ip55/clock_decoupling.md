...
## ✅ 2026-06-25 — Phase-1 graft DONE on the canonical desktop kernel `unix.ip55.g`

The deferred graft above is now applied. `time_override.o` (compiled `-DIP55` on irix-devel) is merged ahead of `IP55bootarea/kernel.o` (`ld -n32 -r`, pristine `kernel.o.preclock` kept for idempotency), then `do_desktop_relink_ip55.sh` relinks `/unix.ip55.g` (now 8810252 B vs the ungrafted 8808544 B; cksum `1862949545`). The pre-graft kernel is saved as `ip55_desktop_kernel/unix.ip55.g.preclock`.

- **ACCEPTANCE TEST PASSED:** `IP55_CPU_HZ=800000000` → `date` advances **0.988× real** (was **2.09×** ungrafted at the same freq). The modeled CPU frequency is now cosmetic for the wall clock on the desktop kernel, not just on `unix.mp.rt`. Boots clean to console login at `-smp 4`.
- **Mechanics (reusable):** the whole build is in `ip54_tftp_staging/dograft_ip55.sh` (so the serial ~256-char truncation never bites — that bit the first attempt); driven by `tmp/ip55-clockdecouple/graft.py` (boot+build) and re-pulled with **per-piece cksum** verification (`repull.py`) after the size-only pull let a corrupt gzip piece through. ⚠️ Gotcha for future pull scripts: a regex like `(\d+)\s+(\d+)\s+/unix\.ip55\.g` matches the `ls -l` **date** (`16  2026`), not the `cksum` line — anchor on the `cksum` output explicitly.
- **Still open:** Phase-2 (100 Hz tick) + Phase-3 (userspace UST page) — the tick/UST still scale with the modeled freq, so keep `IP55_CPU_HZ` at the 66.67 MHz default for the shipped desktop until those land. Phase-1 (gettimeofday/`date`/networking timestamps) is complete.
